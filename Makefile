# --------------------------------------------------------------------
# This Makefile was borrowed from https://github.com/LINKIT-Group/dockerbuild
# Read the blog post here: https://itnext.io/docker-makefile-x-ops-sharing-infra-as-code-parts-ea6fa0d22946
# --------------------------------------------------------------------
...
.PHONY: shell help build rebuild up down login test clean prune
...
help:
	...
	@echo '  up		run as service --container-- for current user: $(HOST_USER)(uid=$(HOST_UID))'
	@echo '  down		bring down docker --container-- for current user :$(HOST_USER)(uid=$(HOST_UID))'
...
up:
	# bring up the app and the database
	docker-compose -p $(PROJECT_NAME)_$(HOST_UID) up $(SERVICE_TARGET) tsappdb
